   ...
   if(squareState==SQUARE_BREAKOUT_WAITING_TAKE)
     {
      ...
      //--- CORREÇÃO: Só verificar barras que aconteceram DEPOIS de detectar o breakout
      //--- Isso evita detecção retroativa de entradas que ocorreram antes de tocar o nível de ativação
      int checkStart = MathMax(breakoutBar+1, breakoutDetectedAtBar+1);
      int checkEnd=current_bar;

      //--- Check if a candle closed beyond the square top AFTER the breakout
      bool closedBeyondTop=false;
      for(int i=checkStart; i<=checkEnd; i++)
        {
         if(is_bullish)
           {
            //--- For bullish: check if close is ABOVE square top
            if(close[i]>squareTop)
              {
               closedBeyondTop=true;
               takeActivationClosePrice=close[i];
               //--- Check if still within entry limit
               double maxEntryPrice=squareTop+InpEntryLimit*_Point;
               if(takeActivationClosePrice<=maxEntryPrice)
                 {
                  //--- Activate take profit
                  takeActivated=true;
                  takeProfitPrice=takeActivationClosePrice+InpTakeProfit*_Point;
                  squareState=SQUARE_TAKE_ACTIVE;
                  Print("═══ TAKE ACTIVATED (BULLISH) ═══");
                  Print("Bar: ", i);
                  Print("Close price: ", DoubleToString(takeActivationClosePrice, _Digits));
                  Print("Square Top: ", DoubleToString(squareTop, _Digits));
                  Print("Max Entry Price: ", DoubleToString(maxEntryPrice, _Digits));
                  Print("Take profit price: ", DoubleToString(takeProfitPrice, _Digits));
                  break;
                 }
               else
                 {
                  //--- Outside entry limit, lock square
                  squareState=SQUARE_LOCKED;
                  Print("═══ LOCKED - Outside entry limit (BULLISH) ═══");
                  Print("Bar: ", i);
                  Print("Close price: ", DoubleToString(takeActivationClosePrice, _Digits));
                  Print("Max Entry Price: ", DoubleToString(maxEntryPrice, _Digits));
                  break;
           ...
           {
            //--- For bearish: check if close is BELOW square bottom
            if(close[i]<squareBottom)
              {
               ...
               if(takeActivationClosePrice>=minEntryPrice)
                 {
                  //--- Activate take profit
                  takeActivated=true;
                  takeProfitPrice=takeActivationClosePrice-InpTakeProfit*_Point;
                  squareState=SQUARE_TAKE_ACTIVE;
                  Print("═══ TAKE ACTIVATED (BEARISH) ═══");
                  Print("Bar: ", i);
                  Print("Close price: ", DoubleToString(takeActivationClosePrice, _Digits));
                  Print("Square Bottom: ", DoubleToString(squareBottom, _Digits));
                  Print("Min Entry Price: ", DoubleToString(minEntryPrice, _Digits));
                  Print("Take profit price: ", DoubleToString(takeProfitPrice, _Digits));
                  break;
                 }
               else
                 {
                  //--- Outside entry limit, lock square
                  squareState=SQUARE_LOCKED;
                  Print("═══ LOCKED - Outside entry limit (BEARISH) ═══");
                  Print("Bar: ", i);
                  Print("Close price: ", DoubleToString(takeActivationClosePrice, _Digits));
                  Print("Min Entry Price: ", DoubleToString(minEntryPrice, _Digits));
                  break;
