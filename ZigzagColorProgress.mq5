...
enum SquareState
  {
   ...
   SQUARE_LOCKED       // Locked due to entry limit violation or 110% breach
  };
   ...
   if(squareState==SQUARE_ACTIVE)
     {
      ...
      //--- NOVA REGRA: Verificar qual rompimento ocorre primeiro (110% ou topo do quadrado)
      bool broke110=false;
      bool brokeSquare=false;

      //--- Verificar rompimento de 110% (usar high/low, não close)
      if(is_bullish)
        {
         //--- Para bullish: 110% está abaixo de 100%, então rompe se LOW ultrapassa
         if(low[current_bar]<fibo110Price)
            broke110=true;
         //--- Rompe quadrado se HIGH ultrapassa o topo
         if(high[current_bar]>squareTop)
            brokeSquare=true;
        }
      else
        {
         //--- Para bearish: 110% está acima de 100%, então rompe se HIGH ultrapassa
         if(high[current_bar]>fibo110Price)
            broke110=true;
         //--- Rompe quadrado se LOW ultrapassa o fundo
         if(low[current_bar]<squareBottom)
            brokeSquare=true;
        }

      //--- PRIORIDADE: Se rompeu 110%, trava tudo
      if(broke110)
        {
         squareLockedAt110=true;
         squareState=SQUARE_LOCKED;
        }
      //--- Se rompeu o quadrado (e não rompeu 110%), segue para TRIGGERED
      else if(brokeSquare)
        {
         //--- Verificar se está dentro do limite de entrada
         ...
         if(withinEntryLimit)
           {
            ...
            //--- Salvar preço de entrada (nível de breakout)
            if(is_bullish)
               entryPrice=squareTop;
            else
               entryPrice=squareBottom;

            //--- Calcular take e stop a partir do preço de entrada
            if(is_bullish)
              {
      ...
      //--- Se ainda está ACTIVE (não rompeu nada), desenhar o quadrado
      if(squareState==SQUARE_ACTIVE)
        {
         DrawSquare(time, current_bar, is_bullish);

         //--- Calcular take e stop para exibição (mesmo antes do breakout)
         if(is_bullish)
           {
         ...
         //--- Desenhar linhas de Take Profit e Stop Loss
   ...
   if(squareState==SQUARE_TRIGGERED)
     {
      ...
      if(hitTake || hitStop)
        {
         //--- Travar tudo e esperar nova Fibonacci
         squareState=SQUARE_LOCKED;
         //--- NÃO deletar objetos, manter visível
         //--- NÃO resetar squareUsedForCurrentLeg, apenas quando leg mudar
         return;
        }

      //--- Continuar desenhando quadrado e linhas TP/SL
      DrawSquare(time, current_bar, is_bullish);
      DrawTakeProfitStopLoss(time, current_bar, is_bullish);
     }
   ...
   if(squareState==SQUARE_LOCKED)
     {
      ...
      //--- Calcular take e stop para exibição
      double squareTop, squareBottom;
      if(is_bullish)
        {
         squareBottom=squareBasePrice;
