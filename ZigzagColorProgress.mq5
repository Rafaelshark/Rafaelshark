...
//--- Check if leg changed
   ...
   if(legChanged)
     {
      //--- Quando a leg muda:
      //--- Se está em TRIGGERED (esperando take/stop), manter e esperar
      //--- Se está em LOCKED (110% ou após take/stop), resetar para permitir novo quadrado na nova leg
      //--- Se está em ACTIVE ou WAITING, resetar normalmente
      if(squareState==SQUARE_TRIGGERED)
        {
         //--- Manter quadrado travado esperando take ou stop
         //--- Não resetar, continuar monitorando a mesma leg até take/stop
        }
      else
        {
         //--- Resetar para permitir novo quadrado na nova leg
         squareState=SQUARE_NONE;
         ObjectsDeleteAll(0,squarePrefix);
         ...
         if(touched)
           {
            ...
            last110CheckBar=-1; // Reset 110% check
   ...
   if(squareState==SQUARE_ACTIVE)
     {
      ...
      int endCheckBar=current_bar-1; // Não verifica barra atual
      ...
      if(squareState==SQUARE_ACTIVE)
        {
         ...
         if(is_bullish)
           {
            double potentialEntry=squareBasePrice+(InpSquareHeight*_Point);
